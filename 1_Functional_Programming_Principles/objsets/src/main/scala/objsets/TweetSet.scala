...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    def findMore(tweet1: Tweet, tweet2: Tweet): Tweet =
      if (tweet1.retweets > tweet2.retweets) tweet1 else tweet2

    if (left.isEmpty && right.isEmpty) elem
    else if (left.isEmpty) findMore(elem, right.mostRetweeted)
    else findMore(elem, left.mostRetweeted)
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter {
    (t: Tweet) => google.exists((word: String) => t.text.contains(word))
  }
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter {
    (t: Tweet) => apple.exists((word: String) => t.text.contains(word))
  }

...
object Huffman {
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] = {
      def updateResult(acc: Map[Char, Int], c: Char) = {
        val count = (acc get c).getOrElse(0) + 1
        acc + ((c, count))
      }

      (Map[Char, Int]() /: chars)(updateResult).iterator.toList
    }
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def lookup(tree: CodeTree)(c: Char): List[Bit] = tree match {
        case Leaf(_, _) => List()
        case Fork(s, r, _, _) if chars(s).contains(c) => 0 :: lookup(s)(c)
        case Fork(s, r, _, _) => 1 :: lookup(r)(c)
      }
